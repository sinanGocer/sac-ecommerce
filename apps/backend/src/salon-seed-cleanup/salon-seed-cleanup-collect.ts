...
/**
 * Salon Seed Cleanup — READ-ONLY durum toplayıcı (tek kaynak).
 *
 * Hem dry-run hem commit script'i bu modülü kullanır; böylece iki yol da
 * BİREBİR aynı snapshot + referans sınıflandırmasını üretir ve aynı
 * plan_fingerprint'i hesaplar. Bu modül yalnız `query.graph` ve projection
 * servisinin LIST metodunu çağırır — hiçbir mutation/DB write yapmaz.
 */
...
export interface CollectedSalonSeedState {
  snapshots: SeedProductSnapshot[]
  countsByProductId: Record<string, ProductReferenceCounts>
}

export async function collectSalonSeedState(
  container: ExecArgs["container"]
): Promise<CollectedSalonSeedState> {
 ...
 */
export function isSafeTestOrder(order: {
